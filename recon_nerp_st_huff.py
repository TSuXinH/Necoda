...
from auxiliary import (ndarray2tif_min_max_clip, ndarray2tif_mean_std_clip, video_resize,
                       CONVERT_UINT16_FLOAT64, ndarray2tif_mean_max_clip, create_overlap_patch_info_test, get_interp_coord)
...
def main():
    ...
    parser.add_argument('--do_resize', action='store_true', default=False, help='Specify whether do resize for the dataset')
    ...
    emb_storage_path = os.path.join(args.ckpt_store_dir, 'g1')
    ...
    args_dict = torch.load(f'{huff_storage_path}/args.pth')
    model = NeRPSTPro(
    ...
    for item in args.generalization_id_list:
        ...
        np_res = np.zeros(shape=(args_dict_m['t'], args_dict_m['x'],
                                 args_dict_m['y'] if 'y' in args_dict_m.keys() else args_dict_m['x']), dtype=np.float64)
        ...
        for patch_idx in range(len(vid_embed_s)):
            ...
            start_y, end_y = get_interp_coord(cur_coord[2], args_dict_m['y'], args_dict_m['patch_x'], args_dict_m['interp_size_x'])
        ...
        if 'tif_min' in args_dict_m.keys():
            print('Current min and max: {}, {}'.format(args_dict_m['tif_min'], args_dict_m['tif_max']))
            tif_res = ndarray2tif_min_max_clip(np_res, tif_min=args_dict_m['tif_min'], tif_max=args_dict_m['tif_max'])
        elif 'tif_max_minus_mean' in args_dict_m.keys():
        ...
        elif 'tif_std' in args_dict_m.keys():
            print('Current mean and std: {}, {}'.format(args_dict_m['tif_mean'], args_dict_m['tif_std']))
            tif_res = ndarray2tif_mean_std_clip(np_res, tif_mean=args_dict_m['tif_mean'], tif_std=args_dict_m['tif_std'])
        else:
        ...
        # tif.imwrite(out_vid.replace(args.name, 'overlap'), np.reciprocal(mode_res).astype(np.uint16))
