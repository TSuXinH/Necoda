...
id_abo = '576757012'
data_path = '/slfm/xxh/others/proj_dc/data/ABO/{}/movie/00000.tif'.format(id_abo)
result_root = '/slfm/xxh/others/proj_dc/exp/nerp_st_huff/ABO/{}'.format(id_abo)
gen_data_path_all = ''
for idx in range(1, 2):
    idx_str = str(idx).zfill(2)
    gen_data_path_all += '/slfm/xxh/others/proj_dc/data/ABO/{}/movie/000{}.tif '.format(id_abo, idx_str)
...
for loss in ['L2']:
    for e in [100]:
        for b in [2]:
            for ef in [10]:
                for lr in [2e-4]:
                    os.system("python train_hierarchy.py --pre_norm mean_max --output_path {} --data_path {} \
                    ...
                    --lam_temporal {}  --lam_spatial {} --quant_embed_bit 4 --remark interp8 --interp_size_x 8 \
                     --interp_size_t 8 --hierarchy_epochs 20 --remark hi \
