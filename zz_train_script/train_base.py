...
from model_box import NeRPBase
...
def main():
    ...
    parser.add_argument('--emb_dim', type=int, default=8, help='embedding dim')
    parser.add_argument('--s_rate_list', type=int, nargs='+', default=[],
                        help='spatial sample rate list')
    parser.add_argument('--t_rate_list', type=int, nargs='+', default=[],
                        help='temporal sample rate list')
    parser.add_argument('--chns_list', type=int, nargs='+', default=[], help='channels list')
    parser.add_argument('--model_type', type=str, default='raw', choices=['nerv', 'hnerv', 'diff', 'nerp', 'nerp_st', 'nerp_base'],
    ...
    exp_id = (
        ...
        f'_s{np.prod(args.s_rate_list).item()}t{np.prod(args.t_rate_list).item()}'
    )
...
def train(local_rank, args):
    ...
    print('GENERATE TRAINING DATALOADER -->')
    ...
    print('GENERATE TESTING DATALOADER -->')
    ...
    print('GENERATE GENERALIZATION DATALOADER LIST -->')
    ...
    encoder_param = ((sum([p.data.nelement() for p in model.st_encoder.parameters()]) / 1e6)
                     + (sum([p.data.nelement() for p in model.pass_way.encoder.parameters()]) / 1e6))
    decoder_param = ((sum([p.data.nelement() for p in model.st_decoder.parameters()]) / 1e6)
                     + (sum([p.data.nelement() for p in model.pass_way.decoder.parameters()]) / 1e6))
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        for i, sample in enumerate(train_dataloader):
            ...
            patch_out, _ = model(patch_data)
            final_loss = F.smooth_l1_loss(patch_out, patch_gt, args.loss)
            raw_loss_list.append(final_loss.detach().item())

            if args.lam_perceptual:
                patch_out_temporal_averaging = torch.mean(patch_out, dim=2)
                patch_gt_temporal_averaging = torch.mean(patch_gt, dim=2)
                perceptual_loss = perceptual_part(patch_out_temporal_averaging, patch_gt_temporal_averaging)
            ...
            if args.lam_temporal:
                spatial_averaging_loss = F.smooth_l1_loss(torch.mean(patch_out, dim=[3, 4]),
...
def evaluate(model, test_dataloader, local_rank, args, epoch=1, huffman_coding=False, generalization_id=1):
    ...
    for model_ind, cur_model in enumerate(model_list):  # model_index 0: raw model, 1: quantized model
        ...
        for i, sample in enumerate(test_dataloader):
            patch_data = data_to_gpu(sample['patch'], device)
            patch_out, emb = cur_model(
                patch_data,
                dequant_vid_embed[i] if model_ind else None,
            )
        ...
        if model_ind == 0:
            vid_emb = torch.cat(emb_list, 0)  # concatenate all the encoded embedding.
            quant_emb, dequant_emved = quant_tensor(vid_emb, args.quant_embed_bit)
    ...
    if quant_ckt is not None:
        ...
        quant_all = {'quant_ckpt': quant_ckt, 'quant_embed': quant_emb, 'm_args': args}
        raw_all = {'raw_all': raw_ckt, 'raw_embed': vid_emb, 'm_args': args}
