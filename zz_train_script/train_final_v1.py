...
def main():
    ...
    parser.add_argument('--interp_method', type=str, default='interp', choices=['interp', 'conv'],
                        help='select methods for interpolating for overlap')
    ...
    # todo: test the other normalization methods
    ...
    if args.overwrite and os.path.isdir(args.output_path):
        choice = input(
            'Do you want to delete previous results ? Key 1 for doing that, else the program will be shut down: ')
        if choice == '1':
...
def train(local_rank, args):
    ...
    hyper_param_dict = {
        'patch_x': args.patch_x,
        'patch_t': args.patch_t,
        'interp_size_x': args.interp_size_x,
        'interp_size_t': args.interp_size_t,
        'interp_chn': args.interp_chn,
        'pre_s_rate': args.pre_s_rate,
        'pre_t_rate': args.pre_t_rate,
    ...
    torch.save(hyper_param_dict, '{}/hyper_param.pth'.format(args.output_path))
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        for i, sample in enumerate(train_dataloader):
            ...
            if i % args.print_freq == 0 or i == len(train_dataloader) - 1:
                ...
                print_str = (
                    '[{}] Rank:{}, Epoch[{}/{}], Step [{}/{}], lr:{:.2e}, '
                    'pred_PSNR: {}, raw l2 loss: {:.6e}, perceptual loss: {:.6e}').format(
        ...
        if epoch == args.epochs - 1:
            ...
            # todo: unify results print and log
...
def evaluate(model, evaluate_data_loader, local_rank, args, epoch=1, generalization_id=1):
    ...
    for model_ind, cur_model in enumerate(model_list):  # model_index 0: raw model, 1: quantized model
        ...
        for i, sample in enumerate(evaluate_data_loader):
            ...
            if i % args.print_freq == 0 or i == len(evaluate_data_loader) - 1:
                print_str = '[{}] Rank:{}, Eval at Step [{}/{}] '.format(datetime.now().strftime("%Y/%m/%d %H:%M:%S"),
                                                                         local_rank, i + 1, len(evaluate_data_loader))
    ...
    raw_all = {'raw_embed_s': vid_emb_s, 'raw_embed_t': vid_emb_t, 'm_args': args}
    ...
    if args.quant_embed_bit == 4:
        ...
        set_dir(cur_gen_huff_dir)
        args_dict = {
            'x': args.x,
            'y': args.y,
            't': args.t,
            'tif_mean': args.tif_mean,
            'tif_max_minus_mean': args.tif_max_minus_mean,
            's_min': quant_emb_s['min'],
            's_scale': quant_emb_s['scale'],
            't_min': quant_emb_t['min'],
            't_scale': quant_emb_t['scale'],
            's_size': quant_emb_s['quant'].shape,
            't_size': quant_emb_t['quant'].shape
        }
        # todo: 1, write a 7z zip to compress all the content before it is stored as a folder, test before writing
...
def quant_model(model, args):
    ...
    if args.quant_model_bit == -1:
        ...
        encoder_k_list = []
        for k, v in cur_ckt.items():  # remember that all keys are still maintained in the dict, so the next step of removal is necessary
            if 'encoder' in k:
