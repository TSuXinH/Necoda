...
def main():
    ...
    # parser.add_argument('--img_chns', type=int, default=3, help='the input and output channels of image, e.g. 1 for gray and 3 for rgb')
...
def train(local_rank, args):
    ...
    decoder_param = ((sum([p.data.nelement() for p in model.st_decoder.parameters()]))
                     + (sum([p.data.nelement() for p in model.st_decoder.parameters()]))
