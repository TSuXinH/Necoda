...
def main():
    ...
    parser.add_argument('--data_path', type=str, default='', help='data path for training input')
    parser.add_argument('--output_path', type=str, default='', help='folder for model checkpoints')
    ...
    parser.add_argument('--patch_x', type=int, default=128, help='patch split x, y size')
    parser.add_argument('--patch_t', type=int, default=128, help='patch split t size')
    parser.add_argument('--gap_x', type=int, default=64, help='training x, y overlap')
    parser.add_argument('--gap_t', type=int, default=64, help='training t overlap')
    ...
    parser.add_argument('--apply_augmentation', action='store_true', help='apply augmentation for the raw and target')
    parser.add_argument('--apply_sampling', action='store_true', default=False, help='apply hierarchical sampling')
    parser.add_argument('-g', '--generalization_data_path', type=str, nargs='+', default=[],
                        help='import generalization data for testing, specify all of the data paths')
    ...
    parser.add_argument('--pre_norm', type=str, default='mean_std', choices=['min_max', 'mean_std', 'mean_max'],
                        help='determine the pre processing normalization of videos before being sent to the network')
    parser.add_argument('--pre_s_rate', type=int, default=2,
                        help='st encoder/decoder, spatial shrinking/expanding rate')
    parser.add_argument('--pre_t_rate', type=int, default=2,
                        help='st encoder/decoder, temporal shrinking/expanding rate')
    ...
    if args.overwrite and os.path.isdir(args.output_path):
        choice = input('Do you want to delete the previous folders or potential results ? Key 1 for doing that, else not and the program will be shut down: ')
        if choice == '1':
...
def train(local_rank, args):
    ...
    encoder_param = ((sum([p.data.nelement() for p in model.interp_encoder.parameters()]))
                     + (sum([p.data.nelement() for p in model.st_encoder.parameters()]))
                     + (sum([p.data.nelement() for p in model.t_pass_way.encoder.parameters()]))
                     + (sum([p.data.nelement() for p in model.s_pass_way.encoder.parameters()]))) / 1e6
    decoder_param = ((sum([p.data.nelement() for p in model.st_decoder.parameters()]))
                     + (sum([p.data.nelement() for p in model.interp_decoder.parameters()]))
                     + (sum([p.data.nelement() for p in model.t_pass_way.decoder.parameters()]))
...
def Dump2CSV(args, best_results_list, results_list, psnr_list, filename='results.csv'):
    result_dict = {'CurEpoch': args.cur_epoch, 'Time': args.train_time,
                   ...
                   'Lr_type': args.lr_type, 'LR (E-3)': args.lr * 1e3,
